...
mixture_vectors = torch.empty((n_plots, n_plots, 4))
...
for i in range(n_plots):
    for j in range(n_plots):
        ...
        mixture_vectors[i, j, :2] = row_mus[i] * torch.Tensor([1.0 - col_mus[j], col_mus[j]])
        mixture_vectors[i, j, 2:] = (1.0 - row_mus[i]) * torch.Tensor([1.0 - col_mus[j], col_mus[j]])
    # end for
# end for
...
for i in range(n_plots):
    for j in range(n_plots):
        ...
        # Add mixed conceptor and use it
        conceptors.add(4, conceptors.morphing(mixture_vector))
        conceptors.set(4)
        ...
        # Generate sample
        generated_sample = conceptor_net(torch.zeros(1, morphing_length + morphing_washout, 1, dtype=dtype), reset_state=False)
        # plt.plot(generated_sample[0, :, 0])
        # plt.show()
        # Save outputs
        print(plots.size())
...
for i in range(n_plots):
    for j in range(n_plots):
        ...
        # Y label
        if j == 0:
            plt.ylabel(row_mus[i])
        # end if

        # X label
        if i == n_plots - 1:
            plt.xlabel(col_mus[j])
        # end if
