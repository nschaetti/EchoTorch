...
TORCH_OPS_IMPLEMENTED = [
    ...
    'permute', 'flatten', 'unbind', 'unsqueeze', 'vsplit', 'vstack', 'where',
    # Pointwise operations,
    'abs', 'absolute', 'acos', 'arccos', 'acosh', 'arccosh', 'add', 'addcdiv', 'addcmul', 'angle', 'asin', 'arcsin',
    'asinh', 'arcsinh', 'atan', 'arctan', 'atanh', 'arctanh', 'atan2', 'bitwose_not', 'bitwise_and', 'bitwise_or',
    'bitwise_xor', 'ceil', 'clamp', 'clip', 'conj', 'copysign', 'cos', 'cosh', 'deg2rad', 'div', 'divide', 'digamma',
    ...
    'rsqrt', 'sigmoid', 'sign', 'sgn', 'signbit', 'relu', 'matmul',
    # Other operations,
    'atleast_1d', 'atleast_2d', 'atleast_3d', 'bincount', 'block_diag', 'broadcast_tensors', 'broadcast_to',
    'bucketize', 'cartesian_prod', 'cdist', 'clone', 'detach', 'combinations', 'cross', 'cummax', 'cummin',
    # Reduction ops
...
class TimeTensor(BaseTensor):
    ...
    def after_permute(
        ...
        r"""After :func:`torch.permute`, map time dimension to its new index."""
        if isinstance(dims, tuple):
            dims = list(dims)
        return TimeTensor(
            data=func_output,
            time_dim=dims.index(self.time_dim)
        )
    # end after_permute
    ...
    def after_flatten(
        ...
        r"""After :func:`torch.flatten`, keep TimeTensor only when time dimension is untouched."""
        in_ndim = input.ndim
        if end_dim < 0:
            end_dim += in_ndim

        if start_dim <= self.time_dim <= end_dim:
            return func_output

        removed_before = 0
        if end_dim < self.time_dim:
            removed_before = end_dim - start_dim

        return TimeTensor(
            data=func_output,
            time_dim=self.time_dim - removed_before
        )
    # end after_flatten
    ...
    def after_reshape(
        ...
        r"""After :func:`torch.reshape`, preserve TimeTensor only for safe mappings."""
        if func_output.ndim > self.time_dim and func_output.size(self.time_dim) == self.tlen:
            return TimeTensor(data=func_output, time_dim=self.time_dim)

        if func_output.ndim > 0 and func_output.size(0) == self.tlen:
            return TimeTensor(data=func_output, time_dim=0)
    ...
    def transform_to_timetensors(
        ...
        """
        return [TimeTensor(o, time_dim=self.time_dim) for o in tensors]
    ...
    def convert_after_reduction(
        ...
        """
        # Normalize dim specification
        def normalize_dims(ndim, red_dim):
            if red_dim is None:
                return None
            if isinstance(red_dim, tuple) or isinstance(red_dim, list):
                dims = []
                for d in red_dim:
                    d = int(d)
                    if d < 0:
        ...
        red_dims = normalize_dims(input.ndim, dim)

        # Full reduction: no temporal semantics to preserve
        if red_dims is None:
            return func_ret

        # Reduction over time dimension removes temporal axis when keepdim=False
        if (self.time_dim in red_dims) and not keepdim:
            return func_ret

        # Compute output time dimension
        if keepdim:
            out_time_dim = self.time_dim
        else:
            removed_before = 0
            for d in red_dims:
                if d < self.time_dim:
                ...
                # end if
            # end for
            out_time_dim = self.time_dim - removed_before
        # end if

        # Convert nested outputs (tensor / tuple / list / named tuple)
        def convert_output(obj):
            if isinstance(obj, torch.Tensor):
                if obj.ndim > out_time_dim:
                    return TimeTensor(data=obj, time_dim=out_time_dim)
                else:
                    return obj
                # end if
            elif isinstance(obj, list):
                return [convert_output(x) for x in obj]
            elif isinstance(obj, tuple):
                values = tuple(convert_output(x) for x in obj)
        ...
        return convert_output(func_ret)
    ...
    def __torch_function__(
        ...
        # Find first timetensor instance in nested args/kwargs
        def first_timetensor(value):
            if isinstance(value, TimeTensor):
                return value
            elif isinstance(value, tuple) or isinstance(value, list):
                for item in value:
                    found = first_timetensor(item)
                ...
                return None
            # end if
        # end first_timetensor

        ttensor = first_timetensor(args)
        if ttensor is None:
            ttensor = first_timetensor(kwargs)
        # end if
        ...
        def convert(value):
            if isinstance(value, TimeTensor):
                ...
                return tuple([convert(a) for a in value])
            elif isinstance(value, list):
                return [convert(a) for a in value]
            elif isinstance(value, dict):
        ...
        # Validate ops inputs
        if ttensor is not None and hasattr(ttensor, 'validate_' + func.__name__):
            getattr(ttensor, 'validate_' + func.__name__)(*args, **kwargs)
        # end if

        # Before callback
        if ttensor is not None and hasattr(ttensor, 'before_' + func.__name__):
            args = getattr(ttensor, 'before_' + func.__name__)(*args, **kwargs)
        # end if

        # Get the tensor in the arguments
        conv_args = [convert(a) for a in args]
        conv_kwargs = convert(kwargs)

        # Middle callback
        if ttensor is not None and hasattr(ttensor, 'middle_' + func.__name__):
            args = getattr(ttensor, 'middle_' + func.__name__)(*args, **kwargs)
        # end if

        # Execute function
        ret = func(*conv_args, **conv_kwargs)

        # If output can be directly converted to timetensor
        if func.__name__ in TORCH_OPS_DIRECT:
            ret = ttensor.convert_to_timetensor(ret)
        elif func.__name__ in TORCH_OPS_REDUCTION:
            ret = ttensor.convert_after_reduction(ret, *args, **kwargs)
        # end if

        # Create TimeTensor and returns or returns directly
        if ttensor is not None and hasattr(ttensor, 'after_' + func.__name__):
            return getattr(ttensor, 'after_' + func.__name__)(ret, *raw_args, **kwargs)
        else:
            return ttensor.convert_similar_tensors(ret) if ttensor is not None else ret
        # end if
...
class FloatTimeTensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(FloatTimeTensor, self).__init__(data, time_dim=time_dim)
        self.float()


class DoubleTimeTensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(DoubleTimeTensor, self).__init__(data, time_dim=time_dim)
        self.double()


class HalfTimeTensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(HalfTimeTensor, self).__init__(data, time_dim=time_dim)
        self.half()


class BFloat16Tensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(BFloat16Tensor, self).__init__(data, time_dim=time_dim)
        self.bfloat16()


class ByteTimeTensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(ByteTimeTensor, self).__init__(data, time_dim=time_dim)
        self.byte()


class CharTimeTensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(CharTimeTensor, self).__init__(data, time_dim=time_dim)
        self.char()


class BooleanTimeTensor(TimeTensor):
    def __init__(self, data: Union[torch.Tensor, 'TimeTensor'], time_dim: Optional[int] = 0) -> None:
        super(BooleanTimeTensor, self).__init__(data, time_dim=time_dim)
        self.bool()
